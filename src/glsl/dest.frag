...
void main(void) {
  ...
  if(crossTest.a > test) {
    ...
    float contrast = abs(snoise(vec2(time * 0.01, time * -0.01) + vUv * 1.0)) * 1.0;
    // contrast = pow(contrast, 10.0);
    ...
    // destA.a *= f;
    // destA.rgb = mix(destA.rgb, 1.0 - destA.rgb, f);
    float lineScale = 0.75;
    vec2 v = gl_FragCoord.xy * lineScale;
    float f = (sin(v.x + v.y) + 1.0) * 0.5;
    destA.rgb *= f;
  }
